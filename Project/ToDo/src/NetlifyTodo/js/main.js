...
function handleCheckbox(event, id) {
    ...
    if (selectedTodoItems.length > 1 && !document.querySelector(".mark-all-btn")) {
        console.log('************ Show Mark All Complted Button');

        const markAllCompleteBtn = document.createElement("button");
        markAllCompleteBtn.textContent = "Mark Selected Todo";
        markAllCompleteBtn.className = "button button-primary icon-prefix circle-check-icon mark-all-btn";

        markAllCompleteBtn.addEventListener("click", (event) => {
            console.log('*** Should Mark the list Item');

            const updatedTodoList = todoList.map((globalListItem) => {
                const selectedItem = selectedTodoItems.find(selectedItem => globalListItem.id === selectedItem.id);

                if (globalListItem.id === selectedItem?.id) {
                    globalListItem.isCompleted = true;
                }
        ...
        document.querySelector("main").appendChild(markAllCompleteBtn);
    } else if (selectedTodoItems.length == 1) {
    ...
    if (!currentItem.classList.contains("completed")) {
        ...
        textInput.addEventListener("keyup", (event) => {
            if (event.code === "Enter") {
                ...
                todoList[index].todoText = updatedTitle;
